...
async def observe_service_execution_overhead(deployment: dict, service_deployment: ServiceDeploymentBenchmark):
    ...
    logger.info(f"warmup, deployment {deployment_id}")
    ...
    for i in range(0, service_deployment.count):
        logger.info(f"RM service startup {i}, deployment {deployment_id}")
        ids = [sr['resource_deployment_id'] for sr in deployment['service_resources']]
        rm_start_times[i] = await rm_operator.startup_stop_service_deployments(deployment_id, ids,
                                                                               ServiceDeploymentMethod.STARTUP)
    ...
    with open(f"{service_deployment.benchmark_id}.csv", mode="w", newline='') as output:
        ...
        for i in range(0, service_deployment.count):
            writer.writerow([i, 'rm', 'start', str(rm_start_times[i])])
            writer.writerow([i, 'rm', 'stop', str(rm_stop_times[i])])
            writer.writerow([i, 'k8s', 'start', str(k8s_start_times[i])])
            writer.writerow([i, 'k8s', 'stop', str(k8s_stop_times[i])])
