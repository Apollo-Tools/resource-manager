...
const SLOSelection = ({metrics, selectedMetrics, slo, updateMetric, updateExpression}) => {
  ...
  return (
    <Space align="start">
      ...
      >
        {metrics.map((metric) => (
          <Option key={metric.metric_id} value={metric.metric_id} disabled={metricAlreadySelected(metric)}>{metric.metric}</Option>
        ))}
      </Select>
      <Radio.Group name="radiogroup" value={expression} onChange={(e) => onChangeExpression(e.target.value)} className="w-32">
        {expressions[slo.metricType].map((exp, idx) => {
          return <Radio.Button key={idx} value={exp}>{exp}</Radio.Button>;
        })}
      </Radio.Group>
      <SLOValue expression={expression} metricType={slo.metricType} />
    </Space>
  );
};
